...
2. api-library::CHAT_APP_VERSION 에 위 버전 값 보다 1 작은 값을 지정한다.

해설.

* 사용자가 먼저 앱을 설치 할 때, CLIENT 버전이 1111223344 와 같이 업데이트를 한다.
* 그럼 SERVER PHP 버전에는 1 작은 값인, 1111223343 으로 한다.
* 그러면 앱에서는 새로운 버전으로 업데이트를 하라는 표시를 하지 않는다.
* 그리고 앱이 업데이트되면, 2222334455 로 하고,
* PHP 서버 버전은 1 작은 값인 2222334454 로 한다.
* 이렇게 하면, 기존 사용자는 여전히 1111223344 의 값을 가지고 있고,
* 서버 버전은 이미 2222334454 이므로, 앱을 업데이트하라는 표시를 한다.
* 사용자가 앱 업데이트를 하면, 서버의 버전 보다 크므로 앱 업데이트하라는 표시를 하지 않는다.

* 이와 같이 서버의 버전을 클라이언트 버전 보다 1 낮추는 것이 포인트이다.
