...
* @done 않읽은 채팅이 있는 방만 따로 모아서 보기. 카톡에는 이게 없어서 불편했다.
  ...
  * 방입장, 전체방에서 입장, 검색후 입장 등에서, 로딩 타임을 빠르게 할 것. 또는 캐시 된 것을 보여 줄 것.

* 방목록을 할 때, 각 방마다 마지막 글 30개씩 캐시를 보여준다.
  * 그리고 각 방마다 맨 마지막 글을 밑에 보여 준다.
  * 이렇게 하면 방 입장시 캐시를 빠르게 보여 줄 수 있다.
...
* 로그아웃을 하면 전체 방으로 이동. 내 방으로 이동 못하게 할 것.
* 로그인을 하면, 전체 방 목록을 새로 읽는다. (이전 사용자의 방 목록이 보임)
...
* 필고 홈페이지에 각 방별로 공개. 더 많은 채팅 유저를 확 볼 수 있음.
...
* 방 홍보 기능. 포인트로 하며 1 포인트당 1 뷰(고유 접속자)를 필고 홈페이지에 함.
  => 이걸 할려고 하면, 새로운 포인트 광고 기능을 만들어야 한다.
...
### Todo for Next version and Sonub

* 필리핀 커뮤니티 앱. 소너브.
  * 완전한 게시판과 채팅 기반으로 하는 외국인을 위한 필리핀 커뮤니티 앱.

* 필고 챗을 영어 버전으로 해서, sonub chat 을 만든다.

* 내 방 목록에서, 읽지 않은 메시지를, 전체 읽음으로 표시하기 기능.
  ==> 그런데, 읽지 않은 메시지가 있는 방만 목로하는 기능이 있는데, 굳이 이 기능이 필요할까?
...
* topic 푸시 알림이 확실히 좀 문제 있어 보임. 괜찮은 것 같기도 함. 확실히. 확실히. 체크를 할 것.
...
* 특정 채팅방에 각 메시지 별로 총 방 인원 중 몇 명이 읽었는지는 카카오톡 처럼 [실시간 업데이트](.)가 되어야 의미가 있다.
  어려운 점은. 각 메시별로 몇명이 읽었는지 카운트를 하고 실시간으로 업데이트를 해야한다는 것이다.
  누가 방 입장을 하거나 현재 방에 있는 경우, 카운트야 증가하면 되는데,
  실시간으로 업데이트하기 위해서는 socket.io 로 구현하던지 Firebase database realtime 업데이트를 사용해야한다.
