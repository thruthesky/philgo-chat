...
* 2018년 8월 30일 업데이트. 발표.
  * 핵심 내용.
    * 질문게시판과 자유게시판에 채팅 연동.
    * 자유게시판에 새글이 등록되면, 자동으로 채팅방이 개설되고, 모든 채팅 글이 그 글의 덧글로 달린다.
    * 질문 게시판도 마찬가지.
...
* 소너브 버전을 개발하는 중.
  ...
  지금 채팅방 앱의 코드를 활용해서,
    git subtree 로 해서,
      컴포넌트, 서비스, angular-wysiwyg 등을 공유해서,
    philgo-chat 앱과 소너브 앱, 하우스메이드 앱, 필리핀 뉴스앱, 업소록 앱. 5개의 프로젝트를 동시에 만든다.

    업소록은 각종 업소 정보를 블로그 처럼 상세히 올릴 수 있는 개인 게시판형이다.

    모든 앱이 메인 페이지와 메뉴만 조금 다를 뿐 나머지 서브페이지는 동일하게 한다.

  * Firebase 에 새로운 루트 프로젝트가 필요하다. sonub 프로젝트를 생성하고,
    모든 앱이 이 하위로 들어간다.
